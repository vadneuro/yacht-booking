      ...
      <section className="py-24 md:py-32 px-5 md:px-8 relative overflow-hidden">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: 'Алексей К.', text: 'Потрясающая прогулка! Капитан Иван показал невероятные места, куда невозможно добраться пешком. Дети в восторге.', yacht: 'Посейдон', rating: 5 },
              { name: 'Марина С.', text: 'Организовали день рождения на яхте. Всё прошло идеально — от встречи до шампанского на закате. Рекомендую!', yacht: 'Афина', rating: 5 },
              { name: 'Дмитрий П.', text: 'Первый раз на парусной яхте. Спокойно, тихо, ветер в лицо. Совсем другое ощущение моря. Вернёмся обязательно.', yacht: 'Мария', rating: 5 },
            ].map((review, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 30 }}
